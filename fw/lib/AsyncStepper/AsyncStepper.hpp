...
class AsyncStepper {
 public:
  ...
  void RotateAngle(float angle, StepperDirection direction, StepperCallback onFinish = nullptr) {
    Rotate(angle / 360.0f * MotorSteps, direction, onFinish);
  }
  ...
  // {
  // 	RotateInTime(angle / 360.0f * MotorSteps, time, direction, onFinish);
  // }
 ...
 private:
  ...
  uint32_t PulseOnWidth = 4; //4 µS pulse on width
  ...
  void InitRotation(unsigned long speed) {
    ...
    /** TODO:: if we were already rotating we should then check that the new speed is upper or lower and brake or accelerate if needed */
